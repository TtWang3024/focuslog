...
- **Eye breaks.** A [BreakTimer](https://github.com/tom-james-watson/breaktimer-app)-style rest reminder that runs for as long as Obsidian does. Every 20 minutes (yours to change) a small card in the corner of your screen counts down, above every app and without taking the keyboard, with Start now / Snooze / Skip; then the screen fills with your own colours and asks you to look away for 20 seconds, with one button, End early. A status-bar clock to the next break, sounds, and an idle reset round it out. It is independent of the pomodoro: it never pauses a task, a pomodoro, or a Focus Log break.
